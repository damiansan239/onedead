class DockerNames {
  // Marked as readonly so the name pools can't be modified unexpectedly
  private readonly left: string[] = [
    ...
    "competent", "condescending", "confident", "cool", "cranky", "crazy", "dazzling", "determined",
    ...
    "stupefied", "suspicious", "sweet", "tender", "thirsty", "trusting", "unruffled", "upbeat",
  ...
  private readonly right: string[] = [
    ...
    "bhabha", "bhaskara", "black", "blackburn", "blackwell", "bohr", "bouth", "borg", "bose",
    "bouman", "boyd", "brahmagupta", "brattain", "brown", "buck", "burnell", "cannon", "carson",
    "cartwright", "carver", "cerf", "chandrasekhar", "chaplygin", "chatelet", "chatterjee", "chaum",
    "chebyshev", "clarke", "cohen", "colden", "cori", "cray", "curie", "curran", "darwin", "davinci",
    ...
    "greider", "grothendieck", "haibt", "hamilton", "haslett", "hawking", "heisenberg", "hellman",
    "hermann", "herschel", "hertz", "heyrovsky", "hodgkin", "hofstadter", "hoover", "hopper", "hugle",
    "hypatia", "ishizaka", "jackson", "jang", "jemison", "jennings", "jepsen", "johnson", "joliot",
    "jones", "kalam", "kapitsa", "kare", "keldysh", "keller", "kepler", "khayyam", "khorana", "kilby",
    "kirch", "knuth", "kowalevski", "lalande", "lamarr", "lamport", "leakey", "leavitt", "lederberg",
    "lehmann", "lewin", "lichterman", "liskov", "lovelace", "lumiere", "mahavira", "margulis",
    "matsumoto", "maxwell", "mayer", "mccarthy", "mcclintock", "mclaren", "mclean", "mcnulty",
    "meitner", "mendel", "mendeleev", "meninsky", "merkle", "mestorf", "mirzakhani", "montalcini",
    "moore", "morse", "moser", "murdock", "napier", "nash", "neumann", "newton", "nightingale",
    ...
    "rhodes", "ride", "ritchie", "robinson", "roentgen", "rosalind", "rubin", "saha", "sammet",
    "sanderson", "satoshi", "shamir", "shannon", "shaw", "shirley", "shockley", "shtern", "sinoussi",
    "snyder", "solomon", "spence", "stonebraker", "sutherland", "swanson", "swartz", "swirles",
    "taussig", "tesla", "tharp", "thompson", "torvalds", "tu", "turing", "varahamihira", "vaughan",
    "villani", "visvesvaraya", "volhard", "wescoff", "wilbur", "wiles", "williams", "williamson",
    "wilson", "wing", "wozniak", "wright", "wu", "yalow", "yonath", "zhukovsky"
  ];

  // Public getters to preserve the alias API if needed elsewhere
  public get adjectives(): string[] { return this.left; }
  public get surnames(): string[] { return this.right; }

  /**
   			* Internal helper to pick randomly and handle the "boring_wozniak" clause.
   			*/
  private generateName(): string {
    const first = this.left[Math.floor(Math.random() * this.left.length)];
    const second = this.right[Math.floor(Math.random() * this.right.length)];
  ...
  /**
   			* Generates a random docker-style name.
   			* @param appendNumber Adds a random single-digit number (1-9) to the name.
   			*/
  public getRandomName(appendNumber?: boolean | number): string {
    const rand = (appendNumber === true || (typeof appendNumber === 'number' && appendNumber > 0))
      ? String(Math.floor(Math.random() * 9) + 1)
      : "";

    return this.generateName() + rand;
  }
}

// Export a single instance to preserve the original module's behavior
